...
export default class TimeDomainDataAnalyser implements AnalyserInterface {
    node: AnalyserNode;
    ...
    static getFrequencyFromTimeDomainData(dataArray: Float32Array, sampleRate: number) {
        ...
        */
        ...
        if (TimeDomainDataAnalyser.calculateRootMeanSquare(dataArray) < 0.01) {
            // not enough signal
            return 0;
        }
        ...
        for (let offset = 1; offset < maxOffset; offset++) {
            const correlation = TimeDomainDataAnalyser.calculateCorrelation(dataArray, offset);
    ...
    getFrequency(): number {
        const dataArray = new Float32Array(this.node.fftSize);
        this.node.getFloatTimeDomainData(dataArray);
        return TimeDomainDataAnalyser.getFrequencyFromTimeDomainData(dataArray, this.audioContext.sampleRate);
    }
}